...
from config.snowflake import sfOptions
import requests
...
def ingestion(api, table_name):
    ...
    try:
        ...
        if res != None and status == 200:
            ...
            cleaned_dataframe.write.format("snowflake").options(**sfOptions).option("dbtable", f"RAW_SCHEMA.{table_name}").mode("overwrite").save()
